  ...
  getApiData(isRefresher = false) {
    ...
    api.subscribe(
      (next) => {
        ...
        this.sqLite.db
          .executeSql('SELECT * FROM api_content WHERE api_path = ?', ['imprint-data-protection'])
          .then((data) => {
            if (data.rows.length > 1 || data.rows.length < 1) {
              this.sqLite.db
                .executeSql('DELETE FROM api_content WHERE api_path = ?', ['imprint-data-protection'])
                .then(() => {
                  this.sqLite.db
                    .executeSql('INSERT INTO api_content (api_path, api_data) VALUES(?,?)', [
                      'imprint-data-protection',
                      JSON.stringify(this.imprintDataProtection),
                    ])
                    .then(() => {})
