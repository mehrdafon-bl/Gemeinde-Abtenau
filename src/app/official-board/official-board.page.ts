    ...
    getApiData(isRefresher = false) {
        ...
        api.subscribe(
            next => {
                ...
                this.sqLite.db.executeSql('SELECT * FROM api_content WHERE api_path = ?', ['official-board'])
                    .then((data) => {
                        if (data.rows.length > 1 || data.rows.length < 1) {
                            this.sqLite.db.executeSql('DELETE FROM api_content WHERE api_path = ?', ['official-board'])
                                .then(() => {
                                    this.sqLite.db.executeSql(
                                        'INSERT INTO api_content (api_path, api_data) VALUES(?,?)',
                                        ['official-board', JSON.stringify(this.boardItems)])
                                        .then(() => {
                                        })
                            ...
                            this.sqLite.db.executeSql(
                                ...
                                [JSON.stringify(this.boardItems), 'official-board'])
                                .then(() => {
                                })
